...
from vuer.schemas import Scene, CameraView
...
class TeleVuer:
    ...
    def _vuer_run(self):
        try:
            ...
            print("[TeleVuer] Vuer process is terminating.")
    ...
    def _xr_render_loop(self):
        while not self.stop_writer_event.is_set():
            ...
            # print("[TeleVuer] Updating XR display frame.")
    ...
    async def main_image_binocular_webrtc(self, session):
        logging.info("[TeleVuer] Starting main_image_binocular_webrtc")
        if self.use_hand_tracking:
            session.upsert(
                Hands(
                    stream=True,
    ...
    async def main_image_monocular_webrtc(self, session):
        logging.info("[TeleVuer] Starting main_image_monocular_webrtc")
        if self.use_hand_tracking:
            session.upsert(
                Hands(
                    stream=True,
    ...
    async def main_image_binocular_webrtc_ego(self, session):
        logging.info("[TeleVuer] Starting main_image_binocular_webrtc_ego")
        if self.use_hand_tracking:
            session.upsert(
                Hands(
                    stream=True,
    ...
    async def main_image_monocular_webrtc_ego(self, session):
        logging.info("[TeleVuer] Starting main_image_monocular_webrtc_ego")
        session.set @ Scene(
            on_head_move=True,
            children=[...],
        )
