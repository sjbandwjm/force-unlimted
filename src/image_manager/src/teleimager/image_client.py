...
import logging
# import logging_mp
# logging = logging_mp.get_logger(__name__, level=logging_mp.INFO)
...
class ZMQ_PublisherThread(threading.Thread):
    ...
    def send(self, data: Any) -> None:
        ...
        try:
            ...
            logging.warning(f"Publisher queue full for {self._host}:{self._port}, dropping message")
        except Exception as e:
            logging.error(f"Error serializing data for publisher: {e}")
    ...
    def stop(self) -> None:
        ...
        self.join(timeout=1)
        if self.is_alive():
            logging.warning("Publisher thread did not stop gracefully")
    ...
    def run(self) -> None:
        ...
        try:
            ...
            while self._running:
                try:
                    ...
                    try:
                        ...
                        logging.warning(f"High water mark reached for at {self._host}:{self._port}, dropping message")
                    except zmq.ZMQError as e:
                        logging.error(f"Failed to publish to at {self._host}:{self._port}: {e}")
                        break
            ...
            logging.error(f"Failed to initialize publisher socket: {e}")
        finally:
...
class ZMQ_PublisherManager:
    ...
    def _create_publisher_thread(self, port: int, host: str = "0.0.0.0") -> ZMQ_PublisherThread:
        try:
            ...
            logging.error(f"Failed to create publisher thread for {host}:{port}: {e}")
            raise
        ...
        with self._lock:
            if key in self._publisher_threads:
                try:
                    ...
                    logging.error(f"Error stopping publisher at {key[0]}:{key[1]}: {e}")
    ...
    def publish(self, data: Any, port: int, host: str = "0.0.0.0") -> None:
        ...
        try:
            ...
            logging.error(f"Unexpected error in publish: {e}")
            raise
...
class ZMQ_SubscriberThread(threading.Thread):
    ...
    def _decode_image(self, jpg_bytes):
        ...
        try:
            ...
            logging.warning(f"[ZMQ_SubscriberThread] Failed to decode image: {e}")
            return None
    ...
    def stop(self) -> None:
        ...
        if self.is_alive():
            logging.warning("Subscriber thread did not stop gracefully")
        ...
        try:
            ...
            while self._running:
                ...
                if self._socket in events:
                    ...
                    logging.debug(f"No message received from {self._host}:{self._port} within timeout.")
        except Exception as e:
...
class ZMQ_SubscriberManager:
    ...
    def _create_subscriber_thread(self, host: str, port: int) -> ZMQ_SubscriberThread:
        try:
            ...
            logging.error(f"Failed to create subscriber thread for {host}:{port}: {e}")
            raise
        ...
        with self._lock:
            for key, subscriber in self._subscriber_threads.items():
                try:
                    ...
                    logging.error(f"Error stopping subscriber at {key[0]}:{key[1]}: {e}")
...
class ZMQ_Responser:
    ...
    def __init__(self, cam_config, host: str = "0.0.0.0", port: int = 60000):
        ...
        self._thread.start()
        logging.info(f"[Responser] Camera Config Responser initialized at {self._host}:{self._port}")
    ...
    def _run(self):
        ...
        while self._running:
            try:
                ...
                logging.error(f"ZMQError in Responser: {e}")
            except Exception as e:
                logging.error(f"Unexpected error in Responser: {e}")
    # --------------------------------------------------------
    # public api
    # --------------------------------------------------------
    def get_port(self):
    ...
    def stop(self):
        ...
        if self._thread.is_alive():
            logging.warning("Responser thread did not stop gracefully")
        try:
            self._socket.close()
            self._context.term()
        except Exception as e:
            logging.warning(f"Error closing Responser socket: {e}")
...
class ZMQ_Requester:
        ...
        try:
            ...
            if self._socket in socks and socks[self._socket] == zmq.POLLIN:
                ...
                if cam_config is not None:
                    logging.info(f"Received camera config from server {self._host}:{self._port}")
                    with open(self._config_client_path, "w") as f:
                        yaml.safe_dump(cam_config, f, sort_keys=False, allow_unicode=True)
                    logging.info(f"Saved camera config to local {self._config_client_path}")
            else:
                logging.warning(f"Request to {self._host}:{self._port} timed out or no response, using local config.")
                if os.path.exists(self._config_client_path):
                    try:
                        with open(self._config_client_path, "r") as f:
                            cam_config = yaml.safe_load(f)
                        logging.info(f"Loaded camera config from local {self._config_client_path}")
                    except Exception as e:
                        logging.warning(f"Failed to load local cam_config_client.yaml: {e}")
                elif os.path.exists(self._config_server_path):
                    try:
                        with open(self._config_server_path, "r") as f:
                            cam_config = yaml.safe_load(f)
                        logging.info(f"Loaded camera config from local {self._config_server_path}")
                    except Exception as e:
                        logging.warning(f"Failed to load local cam_config_server.yaml: {e}")
                else:
                    logging.error("No camera configuration file found locally.")
            return cam_config
        except Exception as e:
            logging.error(f"Unexpected error in Requester: {e}")
            return cam_config
    ...
    def close(self):
        ...
        try:
            ...
            logging.warning(f"Error closing Requester socket: {e}")
...
class ImageClient:
    def __init__(self, host="192.168.123.164", request_port=60000):
        ...
        if not self._cam_config['head_camera']['enable_zmq'] and not self._cam_config['head_camera']['enable_webrtc']:
            logging.warning("[Image Client] NOTICE! Head camera is not enabled on both ZMQ and WebRTC.")
    ...
    def close(self):
        ...
        logging.info("Image client has been closed.")
    ...
    while running:
        if cam_config['head_camera']['enable_zmq']:
            ...
            if head_img is not None:
                logging.info(f"Head Camera FPS: {head_fps:.2f}")
                logging.debug(f"Head Camera Shape: {cam_config['head_camera']['image_shape']}")
                logging.debug(f"Head Camera Binocular: {cam_config['head_camera']['binocular']}")
        ...
        if cam_config['left_wrist_camera']['enable_zmq']:
            ...
            if left_wrist_img is not None:
                logging.info(f"Left Wrist Camera FPS: {left_wrist_fps:.2f}")
                logging.debug(f"Left Wrist Camera Shape: {cam_config['left_wrist_camera']['image_shape']}")
        ...
        if cam_config['right_wrist_camera']['enable_zmq']:
            ...
            if right_wrist_img is not None:
                logging.info(f"Right Wrist Camera FPS: {right_wrist_fps:.2f}")
                logging.debug(f"Right Wrist Camera Shape: {cam_config['right_wrist_camera']['image_shape']}")
                cv2.imshow("Right Wrist Camera", right_wrist_img)

        if cv2.waitKey(1) & 0xFF == ord('q'):
            logging.info("Exiting image client on user request.")
            running = False
            # clean up
            client.close()
